...
if(in_array($ext, $ext_boleh)){
 ...
 $tujuan = 'kategori_image/women/ac/' . $foto['name'];
 move_uploaded_file($sumber, $tujuan);

 $k = new PDO("mysql:host=localhost;dbname=onlineshop","root","");
 $sql = "INSERT INTO img_women (nama, harga, foto, kategori) VALUES (?, ?, ?,'AC')";

 $result = $k->prepare($sql);
 $result->execute([$nama, $harga, $tujuan]);

 
echo "<script>alert('File berhasil dimasukkan');window.location='form_women_ac.php'</script>";
}else{
 echo "<script>alert('File tidak valid');window.location='form_women_ac.php'</script>";
}
